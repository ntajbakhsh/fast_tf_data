...
from utils import py_gen_p1
from utils import py_gen_p2
...
# Note: this solution is optimal.

# set up tf generator
Dataset = tf.data.Dataset
ds = Dataset.from_tensor_slices([0, 1, 2])
ds = ds.interleave(lambda x: Dataset.from_generator(py_gen_p1, output_types=(tf.float32), args=(x,)),
                               cycle_length=3,
                               block_length=1,
                               num_parallel_calls=3)
ds = ds.interleave(lambda x: Dataset.from_generator(py_gen_p2, output_types=(tf.float32), args=(x,)),
                               cycle_length=3,
                               block_length=1,
                               num_parallel_calls=3)
